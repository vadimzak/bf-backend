...
# Configuration
REMOTE_HOST="sample-2.vadimzak.com"
REMOTE_USER="ec2-user"
SSH_KEY="$HOME/.ssh/sample-app-key.pem"
APP_DIR="/var/www/sample-2"
COMPOSE_FILE="docker-compose.prod.yml"
HEALTH_ENDPOINT="https://sample-2.vadimzak.com/health"
IMAGE_NAME="sample-2"
APP_SOURCE_DIR="apps/sample-2"
...
Deploy sample-2 app to production with automated safety checks.
...
commit_and_push() {
    if [[ -n $(git status --porcelain) ]]; then
        ...
        if [[ -z "$COMMIT_MESSAGE" ]]; then
            COMMIT_MESSAGE="Deploy sample-2: $(date '+%Y-%m-%d %H:%M:%S')"
        fi
...
# Stop and start sample-2 service
sudo docker-compose -f $COMPOSE_FILE stop sample-2
sudo docker-compose -f $COMPOSE_FILE up -d sample-2
...
if sudo docker ps | grep -q "sample-2.*healthy"; then
...
post_deployment_health_check() {
    ...
    if curl -s --max-time 10 -I "http://sample-2.vadimzak.com" | grep -q "301"; then
...
cleanup_old_images() {
    ...
    ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$REMOTE_USER@$REMOTE_HOST" << 'EOF'
OLD_REMOTE_IMAGES=$(sudo docker images sample-2 --format "{{.Tag}}" | grep -v latest | tail -n +4)
if [[ -n "$OLD_REMOTE_IMAGES" ]]; then
    echo "Removing old remote images..."
    echo "$OLD_REMOTE_IMAGES" | while read tag; do
        sudo docker rmi sample-2:$tag 2>/dev/null || true
    done
fi

# Clean up old compressed image files
sudo rm -f /tmp/sample-2-*.tar.gz
EOF
...
main() {
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
        if post_deployment_health_check; then
            ...
            echo
            echo "Application is running at: https://sample-2.vadimzak.com"
